...
declare global {
  ...
  interface AdventurelandClient {
    get_characters(): OnlineCharacter[];

    is_monster(entity: Entity): entity is MonsterEntity;
    is_character(entity: Entity): entity is CharacterEntity;

    resolving_promise<T>(data: T): Promise<T>;

    sleep(ms: number): Promise<void>;

    calculate_item_value(item: ItemInfo, m?: number): number;

    open_stand(inventoryIndex?: number): Promise<any>;
    close_stand(): Promise<any>;

    get_party(): Record<string, PartyCharacter>;

    is_object(arg: unknown): boolean;
     ...
     * @param slot Second slot to swap/combine.
     */
    swap(destSlot: number, slot: number): Promise<unknown>;
     ...
     * @param count How many items should be in the new slot.
     */
    split(slot: number, count: number): Promise<unknown>;
    ...
    unmap_key(key: string): void;
    map_key(key: string, thing: string, arg?: string): void;
    load_code(nameOrSlot: string | number, onerror?: any): void;

    /**
     * Accept a magiport request from a mage
     * @param name The name of the mage offering a magiport
     */
    accept_magiport(name: string): void;

    /**
     * Accept the party invititation of another character (i.e. join their party)
     * @param name The name of the character offering a party invite
     */
    accept_party_invite(name: string): void;

    /**
     * Accept the request of another character to join your party (i.e. let them join your party)
     * @param name The name of the character to allow in to your party
     */
    accept_party_request(name: string): void;

    /**
     * Activate an item (likely a booster)
     * @param inventoryPosition The position of the item in your inventory
     */
    activate(inventoryPosition: number): void;
     ...
     * @returns A promise containing information about the attack, such as the projectile, or a promise cointaining the error why the attack didn't work
     */
    // TODO: Change the "any" to the promise that this  returns
    attack(target: Entity): Promise<any>;

    /**
     * Buy an item from an NPC. This  can buy things with gold or shells.
     * @param item The name of the item you wish to purchase (`G.items`)
     * @param quantity How many items to buy. The default is to buy one item
     */
    // TODO: Change the "any" to the promise that this  returns
     ...
     */
    // TODO: Change the "any" to the promise that this  returns
    buy_with_gold(item: ItemKey, quantity?: number): Promise<any>;
     ...
     */
    // TODO: Change the "any" to the promise that this  returns
    buy_with_shells(item: ItemKey, quantity?: number): Promise<any>;

    /**
     * Check if you can attack the given target. This  also checks status conditions by calling `parent.is_disabled(character)` which checks statuses such as `rip` and `stunned`.
     * NOTE: If you just want to check the cooldown, consider using `is_on_cooldown("attack")`
     * @param target The target entity to check if you can attack (`parent.entities`)
     * @returns TRUE if the target is attackable, FALSE otherwise.
     */
    can_attack(target: Entity): boolean;

    /**
     * Check if you can heal the given target.
     * @param target The target entity to check if you can heal (`parent.entities`)
     */
    can_heal(target: Entity): boolean;
     ...
     * @returns TRUE if you are not currently transporting, and can transport, FALSE otherwise
     */
    can_transport(entity: Entity): boolean;
     ...
     * @param returns TRUE if not on cooldown, FALSE otherwise.
     */
    can_use(skill: SkillKey): boolean;
     ...
     * @returns TRUE if the door can be used from the given position, FALSE otherwise
     */
    is_door_close(map: MapKey, door: DoorInfo, x: number, y: number): boolean;

    /**
     * Checks if you can use the given door from the given position
     * This  is costly, so only check when is_door_close is true.
     * @param map A given map (from `G.maps`)
     * @param door The given door (from `G.maps[map].doors`)
     * @param x The x position on the map
     * @param y The y position on the map
     * @returns TRUE if the door can be used from the given position, FALSE otherwise
     */
    can_use_door(map: MapKey, door: DoorInfo, x: number, y: number): boolean;
     ...
     * @example to_pretty_float(123456.789) // "123,456.78"
     */
    to_pretty_float(num: number): string;
     ...
     * @example to_pretty_num(123456.789) // "123,456"
     */
    to_pretty_num(num: number): string;
     ...
     * @example to_pretty_num(1234.789, 10000) // "1234.789"
     */
    smart_num(num: number, edge?: number): string;
     ...
     * @example to_shrinked_num(1234567) // "1.2M"
     */
    to_shrinked_num(num: number): string;

    /**
     * Changes the target of the player. Use in association with `get_targeted_monster()`.
     * @param target A given target (from `parent.entities`)
     */
    change_target(target?: Entity): void;

    /**
     * Clears all drawings from the window. Use this  to clean up `draw_circle` and `draw_line`.
     */
    clear_drawings(): void;
     ...
     * @param code The code snippet to run (using eval)
     */
    command_character(character: string, code: string): void;
    ...
    /** Feed this  a value like (character.apiercing - target.armor) and it spits out a multiplier so you can adjust your expected damage */
    damage_multiplier(difference: number): number;
    distance(from: IPosition | PositionReal, to: IPosition | PositionReal): number;

    equip(inventoryPostion: number, slot?: SlotType): Promise<unknown>;

    dismantle(inventoryPostion: number): Promise<unknown>;
     ...
     * @see log if you don't want to escape the text but still stringify objects.
     */
    safe_log(message: any, color?: string): void;
     ...
     * @param color Can be used to override the color of the message. Any CSS color is fine.
     *
     * @warning This  won't escape html. See `safe_log` if you want to log unsafe data.
     *
     * @see safe_log if you want to escape a string or object that might contain html.
     * @see log if you want to stringify an object.
     */
    game_log(message: any, color?: string): void;
    ...
    get_targeted_monster(): MonsterEntity | null;
    get_target(): CharacterEntity | Entity | null;
    get_target_of(entity: Entity): Entity | null;
    get_player(name: string): CharacterEntity;
    ...
    heal(target: Entity): any;
    use_hp_or_mp(): void;
    /** Checks whether or not we can attack other players */
    is_pvp(): boolean;
    is_in_range(entity: Entity, skill?: SkillKey): boolean;
    is_transporting(entity: Entity): boolean;
    is_moving(entity: Entity): boolean;
    is_on_cooldown(skill: string): boolean;

    /** returns true if you are silenced or disabled */
    is_silenced(entity: Entity): boolean;
     ...
     * @param id The ID of a chest (from `parent.chests`)
     */
    loot(id?: string): Promise<any>;

    mssince(date: Date): number;
    reduce_cooldown(skill: SkillKey, ms: number): void;
    respawn(): any;
    /** Quantity defaults to 1 if not set */
    sell(inventoryPostion: number, quantity?: number): any;
    send_cm(to: string | string[], data: any): any;
    send_gold(to: string | { name: string }, amount: number): any;
    send_item(
      to: string | { name: string },
      inventoryPostion: number,
      quantity?: number
      ...
      >
    >;
    send_local_cm(to: string, data: any): any;
    /**
     *
     * @param name name could be a player object, name, or id
     * @param isRequest If isRequest is set to true, it will send a party request
     */
    send_party_invite(name: any, isRequest?: boolean): any;
    send_party_request(name: string): any;

    leave_party(): Promise<void>;
     ...
     * @param value Whatever you want to display.
     */
    show_json(value: any): void;
    set(key: string, value: unknown): unknown;
    set_message(text: string, color?: string): any;
    simple_distance(from: IPosition | PositionReal, to: IPosition | PositionReal): number;

    start_character(characterName: string, codeSlotOrName?: string | number): any;
    /** stops moving, or channeled actions */
    stop(action?: string): any;
    stop_character(name: string): any;
    /** For buying things off players' merchants */
    trade_buy(target: Entity, trade_slot: TradeSlotType, quantity?: number): any;
    /** For selling things to player merchants */
    trade_sell(target: Entity, trade_slot: TradeSlotType, quantity?: number): any;
    transport(map: MapKey, spawn?: number): any;
    unequip(slot: SlotType | TradeSlotType): any;
    ...
    exchange(item_slot: number): Promise<{
      success: boolean;
     ...
     * @param minutes How long the giveaway should last.
     */
    giveaway(slot: number, inventoryPosition: number, q: number, minutes: number): unknown;
     ...
     * @param rid Unique id of the item you want to join the giveaway of
     */
    join_giveaway(characterName: string, slot_name: TradeSlotType, rid: string): unknown;
     ...
     * @param toBooster Type the booster should be of.
     */
    shift(inventoryPosition: number, toBooster: BoosterKey): Promise<ShiftResult>;

    join(eventName: EventKey): Promise<any>;
    /**
     * MOVEMENT METHODS
     */
     ...
     * @param id The key of the npc from G.npcs
     */
    find_npc(id: NpcKey): IPosition;

    /** This  uses move() if it can, otherwise it uses smart_move() */
    xmove(entity: PositionReal): Promise<boolean>;
    xmove(x: number, y: number): Promise<boolean>;
    ...
    //  can_move(position: PositionMovable & { base: EntityBase }): boolean;
     ...
     * @returns TRUE if you can move there, FALSE otherwise
     */
    can_move_to(destination: ICoordReal): boolean;
    /**
     * Checks if you can move your character to the given destination on your current map
     * @param x The x-coordinate that you want to move to
     * @param y The y-coordinate that you want to move to
     * @returns TRUE if you can move there, FALSE otherwise
     */
    can_move_to(x: number, y: number): boolean;
     ...
     * @param y
     */
    move(x: number, y: number): Promise<MoveResponse>;
    ...
    smart_move(
      ...
      /**
       * a callback  with true or false ad the argument depending on smart_move success or failure
       * @deprecated on_done  is an old callback  for compatibility, smart_move also returns a Promise [25/03/20]
       */
      on_done?: () => Promise<void | boolean>
    ): Promise<SmartMoveResponse>;
     ...
     * @returns TRUE if you are not currently transporting, and can walk, FALSE otherwise
     */
    can_walk(entity: Entity): boolean;
